...
export class UsersService {
  ...
  /**
   * Promotes or demotes a regular user's access role (Author or Reader).
   * Note: Evicts the access guard cache key so the changes take effect immediately.
   */
  async updateUserRole(userId: number, role: UserRole): Promise<User> {
    ...
    // 2. Perform the database role update
    const updatedUser = await this.prisma.user.update({
      where: { id: userId },
      data: { role },
    });
    
    // 3. Cache Eviction: Delete the user's access validation from cache-manager
    // This forces AccessGuard to lookup their role from PostgreSQL on their next request.
    const cacheKey = getAccessGuardCacheKey({ id: userId, type: UserType.User });
